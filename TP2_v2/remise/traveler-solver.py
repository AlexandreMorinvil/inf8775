...
def parse_arguments():
    ...
    if not (algorithm in ['glouton', 'progdyn', 'approx']):
        texte = "L'algorithme '" + algorithm + "' n'est pas valide. Les valeurs accesptées sont : glouton, progdyn ou approx"
        raise Exception(texte)
...
"""
def import_data(directory):
    return np.genfromtxt(directory, delimiter=' ', dtype=None, usecols=range(0,2), skip_header=1)

"""
Computation of the solution according to the algorithm provided
"""
def handle_processing(cities_data, algorithm):
    if algorithm == 'glouton':
        return greedy_solve(cities_data, must_compute_time=True)
    elif algorithm == 'progdyn':
        return dynamic_programming_solve(cities_data, must_compute_time=True)
    elif algorithm == 'approx':
        return approximative_solve(cities_data, must_compute_time=True)
    else :
        raise Exception("L'algorithme est invalide.")

"""
Display of the path points
"""
def handle_solution_presentation(must_present, path_points):
    if not must_present :
        return
...
if __name__== "__main__":
    ...
    solution_reached, total_distance, time_used = handle_processing(cities_data, algorithm)

    # Presentation
    handle_solution_presentation(must_present, solution_reached)
